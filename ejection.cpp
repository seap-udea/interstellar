...
int main(int argc,char* argv[])
{
  ...
  double ap1=0.5,ap2=5.0,dap=0.2;
  double logMp1=log10(3e-4),logMp2=log10(1e-2),dlogMp=(logMp2-logMp1)/10.0;

  FILE* fe=fopen("ejection.data","w");
  int n=1;
  for(ap=0.5;ap<=5.0;ap+=dap){
    for(logMp=logMp1;logMp<=logMp2;logMp+=dlogMp){
      Mp=pow(10.0,logMp);
      navg=velocityDistribution(Ms,ap,Mp,Rp,&vmean,&vstd);
